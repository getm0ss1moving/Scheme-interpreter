...
Value IsEq::evalRator(const Value &rand1, const Value &rand2) {
    if(Integer* rand1_ = dynamic_cast<Integer*>(rand1.get())) {
        if(Integer* rand2_ = dynamic_cast<Integer*>(rand2.get())) {
            if(rand1_->n == rand2_->n) {
                return BooleanV(true);
            }
            return BooleanV(false);
        }
        throw RuntimeError("invalid input");
    }else if(Boolean* rand1_ = dynamic_cast<Boolean*>(rand1.get())) {
        if(Boolean* rand2_ = dynamic_cast<Boolean*>(rand2.get())) {
            if(rand1_->b == rand2_->b) {
                return BooleanV(true);
            }
            return BooleanV(false);
        }
        throw RuntimeError("invalid input");
    }else if(Symbol* rand1_ = dynamic_cast<Symbol*>(rand1.get())) {
        if(Symbol* rand2_ = dynamic_cast<Symbol*>(rand2.get())) {
            if(rand1_->s == rand2_->s) {
                return BooleanV(true);
            }
            return BooleanV(false);
        }
